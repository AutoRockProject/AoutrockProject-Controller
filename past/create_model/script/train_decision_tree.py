...
def prepare_data():
    ...
    """
    # Load features
    features_path = 'create_model/output/all_features.csv'
...
def compute_per_user_metrics(y_test, y_pred, test_df):
    ...
    for user in sorted(unique_users):
        ...
        if mask.sum() > 0:
            ...
            print(f"  {user}: Accuracy = {acc:.4f} ({mask.sum()} samples)")
...
def save_results(model, feature_importance, per_user_accuracy, test_acc, X_train, y_train):
    ...
    """
    output_dir = 'create_model/output'
    ...
    summary = {
        ...
        'feature_columns': list(feature_importance['feature'].head(10).values),
...
def main():
    ...
    print("=" * 60)
    print(f"\nResults saved to: create_model/output/")
