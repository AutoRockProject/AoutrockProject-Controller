#!/usr/bin/env python3
"""
Feature Engineering Script for Fighting Game Decision Tree Model (v2)
Filtered version: Main users only (removes noise data)
...
def extract_features(csv_file):
    ...
    Filters to include only main users
    """
    # Read CSV
    ...
    # Filter to include only main users
    df = df[df['username'].isin(MAIN_USERS)]

    if len(df) == 0:
        return None, user1, user2, match_number
...
def main():
    ...
    Filtered to main users only
    """
    csv_files = sorted(glob.glob('20260513/*.csv'))

    all_data = []
    main_users_found = {user: 0 for user in MAIN_USERS}

    print(f"Processing CSV files... (filtering to main users: {', '.join(MAIN_USERS)})")
        ...
        try:
            ...
            if features_df is not None:
                ...
                for user in features_df['user'].unique():
                    if user in main_users_found:
                        count = (features_df['user'] == user).sum()
                        main_users_found[user] += count
        except Exception as e:
            print(f"    Error processing {csv_file}: {e}")
            continue

    if len(all_data) == 0:
        print("ERROR: No data found for main users!")
        return None
    ...
    for user in sorted(MAIN_USERS):
        count = main_users_found[user]
        print(f"  {user}: {count:,}")

    # Save combined features
    output_path = 'create_model2/output/all_features.csv'
    combined_df.to_csv(output_path, index=False)
    print(f"\nSaved filtered features to {output_path}")
